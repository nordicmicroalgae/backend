...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "file",
            help=("Path to CSV file containing biovolumes."),
        )
        parser.add_argument(
            "-e",
            "--encoding",
            default="cp1252",
            help=('Encoding used in CSV file. Default is "cp1252".'),
        )
        parser.add_argument(
            "--provider",
            help=("Provider of biovolumes data. For example: NOMP"),
            required=True,
        )
        parser.add_argument(
            "--collection",
            default="Biovolumes",
            help=('Facts collection name. Default is "Biovolumes".'),
        )
        parser.add_argument(
            "--id-field",
            default="taxon_id",
    ...
    def handle(self, *args, **options):
        ...
        if id_field not in cols:
            raise CommandError(
                ...
                "Possible values are: %s." % (id_field, ", ".join(cols))
            )
        ...
        for row_index, row in enumerate(rows):
            if None in row.values():
                self.stdout.write(
                    self.style.WARNING(
                        "Seems like malformed row: %u. Skipping row." % (row_index + 2)
                    )
                )
                continue
