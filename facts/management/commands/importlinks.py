...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "file",
            help=("Path to CSV file containing external links."),
        )
        parser.add_argument(
            "-e",
            "--encoding",
            default="utf8",
            help=('Encoding used in CSV file. Default is "utf8".'),
        )
        parser.add_argument(
            "--provider",
            help=("Provider of the external links. For example: AlgaeBase"),
            required=True,
        )
        parser.add_argument(
            "--collection",
            default="External Links",
            help=('Facts collection name. Default is "External Links".'),
        )
        parser.add_argument(
            "--id-field",
            default="taxon_id",
    ...
    def handle(self, *args, **options):
        ...
        if len(rows) == 0:
            raise CommandError("Found zero rows in specified CSV file. Aborting.")
        ...
        for field_arg, field_name in required_fields.items():
            if field_name not in cols:
                raise CommandError(
                    ...
                    "Possible values are: %s." % (field_name, field_arg, ", ".join(cols))
                )
        ...
        for row in rows:
            ...
            if external_id == "":
                if verbosity > 1:
                    self.stdout.write(
                        self.style.WARNING(
                            "%s must not be empty. Skipping row." % external_id_field
                        )
                    )
                continue
