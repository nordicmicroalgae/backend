...
DEFAULT_SYNONYMS_FILE = os.path.join(settings.CONTENT_DIR, "species", "synonyms.txt")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(filepath, "r", encoding=encoding) as in_file:
            rows = list(map(transform_row, csv.DictReader(in_file, dialect="excel-tab")))
        ...
        if verbosity > 1:
            self.stdout.write(
                self.style.SUCCESS(
                    "Successfully imported %u synonyms." % number_of_saved_objects
                )
            )
