...
DEFAULT_TAXA_FILE = os.path.join(settings.CONTENT_DIR, "species", "taxa.txt")
...
class SlugGenerator:
    ...
    @property
    def counter(self):
        if self.slug_base not in self.counter_by_base:
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            help=(
                ...
                'If this isn\'t provided, the path "%s" will be used.' % DEFAULT_TAXA_FILE
            ),
        )
        parser.add_argument(
            "-c",
            "--clear",
            action="store_true",
            help=("Clear any existing objects from database before writing new ones."),
        )
        parser.add_argument(
            "-e",
            "--encoding",
    ...
    def handle(self, *args, **options):
            ...
            for taxon_info in taxa_by_id.values():
                if taxon_info["parent_id"] is None:
                    if verbosity > 1:
                        self.stdout.write(
                            self.style.WARNING(
                                "Missing parent for %s." % taxon_info["scientific_name"]
                            )
                        )
                    continue
                ...
                while parent_info is not None:
                    ...
                    if parent_info["parent_id"] == parent_info["id"]:
                        if verbosity > 1:
                            incomplete_classification = list(
                                map(
        ...
        if verbosity > 0:
            self.stdout.write(
                self.style.SUCCESS(
                    "Successfully imported %u taxa." % number_of_saved_objects
                )
            )
