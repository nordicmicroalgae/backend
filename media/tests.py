...
@pytest.mark.skipif(IN_GITHUB_ACTIONS, reason="Skipped in github actions")
...
@pytest.mark.django_db
@pytest.mark.skipif(IN_GITHUB_ACTIONS, reason="Skipped in github actions")
def test_fetch_media_for_children(client):
    # Given a user
    user = get_user_model().objects.create(id=1)
    ...
    # Then only media for the children is returned
    all_retrieved_media = response.json()["media"]
    assert len(all_retrieved_media) == 2
    ...
    assert parent_media.slug not in retrieved_media_slugs


@pytest.mark.django_db
@pytest.mark.skipif(IN_GITHUB_ACTIONS, reason="Skipped in github actions")
def test_fetch_prioritized_media_for_children(client):
    # Given a user
    user = get_user_model().objects.create(id=1)
    ...
    # Given media for all taxon with different priorities
    parent_media = Media.objects.create(
        slug="parent-media", taxon=parent_taxon, created_by=user
    )
    child1_prioritized_media = Media.objects.create(
        slug="child-media1", taxon=child_taxon1, created_by=user, priority=0
    )
    child1_other_media = Media.objects.create(
        slug="child-media2", taxon=child_taxon1, created_by=user, priority=1
    )
    child2_other_media = Media.objects.create(
        slug="child-media3", taxon=child_taxon2, created_by=user, priority=4
    )
    child2_prioritized_media = Media.objects.create(
        slug="child-media4", taxon=child_taxon2, created_by=user, priority=3
    )
    Media.objects.create(slug="other-media", taxon=other_taxon, created_by=user)
    ...
    url = reverse("media-collection-view")
    response = client.get(
        url, {"taxon": "parent-taxon", "children": "True", "priority": "True"}
    )
    assert response.status_code == 200

    # Then only the most prioritized media for the children is returned
    all_retrieved_media = response.json()["media"]
    assert len(all_retrieved_media) == 2

    retrieved_media_slugs = set([media["slug"] for media in all_retrieved_media])
    assert retrieved_media_slugs == {
        child1_prioritized_media.slug,
        child2_prioritized_media.slug,
    }

    assert parent_media.slug not in retrieved_media_slugs
    assert child1_other_media.slug not in retrieved_media_slugs
    assert child2_other_media.slug not in retrieved_media_slugs
