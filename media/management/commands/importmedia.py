...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "--encoding",
            default="utf8",
            help=('Encoding used in the records JSON file. Default is "utf8".'),
        )
    ...
    def handle(self, *args, **options):
        ...
        if len(loaded_records) == 0:
            raise CommandError("Found zero records in specified JSON file. Aborting.")
                ...
                if verbosity > 1:
                    self.stdout.write(
                        self.style.WARNING(
                            ...
                            "it's relation to a taxon will be lost." % taxon_id
                        )
                    )
        ...
        if verbosity > 0:
            self.stdout.write(
                self.style.SUCCESS(
                    "Successfully imported %u objects." % number_of_saved_objects
                )
            )
