...
class AdminSyncDbView(AdminRequiredMixin, View):
    ...
    def post(self, request):
        log_id = "syncdb-{}".format(timezone.now().strftime("%Y-%m-%d_%H-%M-%S"))

        thread = threading.Thread(target=self._syncdb_thread_func, args=[log_id])
        thread.start()
    ...
    def _syncdb_thread_func(self, log_id):
        ...
        with open(get_log_file(log_id), "a", buffering=1) as log_file:
            try:
                call_command("syncdb", verbosity=2, no_color=True, stdout=log_file)
            except CommandError as e:
                log_file.write("Error: %s\n" % str(e))
            except Exception:
                log_file.write("Something went wrong. Sysadmin should check logs.\n")
                raise
            finally:
