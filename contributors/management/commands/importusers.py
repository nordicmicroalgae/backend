...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "--clear",
            action="store_true",
            help=("Clear any existing users from database before writing new ones."),
        )
        parser.add_argument(
            "-e",
            "--encoding",
            default="utf8",
            help=('Encoding used in the users CSV file. Default is "utf8".'),
        )
    ...
    def handle(self, *args, **options):
        ...
        if len(user_rows) == 0:
            raise CommandError("Found zero records in specified CSV file. Aborting.")
        ...
        for user_row in user_rows:
            ...
            user.is_active = to_boolean(user_row.get("is_active", "yes"))
            user.is_staff = to_boolean(user_row.get("is_staff", "yes"))
            user.is_superuser = to_boolean(user_row.get("is_superuser", "no"))

            last_login = parse_datetime(user_row.get("last_login", ""))
            if last_login is not None:
                user.last_login = last_login

            date_joined = parse_datetime(user_row.get("date_joined", ""))
        ...
        try:
            ...
            raise CommandError(
                "Failed to write to database. Import aborted. Error was %s" % e
            )
